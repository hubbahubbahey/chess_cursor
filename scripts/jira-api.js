...
const API_TOKEN =
  ...
  'ATATT3xFfGF0WduD84K1XitfFP-xlzusJdpyZEUN2VexbK1sKHzTmswHk1vDrH6CzyyfafwPVtnL4fFJvn-trJ1U1sJ23BNYBo7ceOBYJxDD7hxrL-gWbIPBiQbJAPg7GndAR8tbXPQMcTnItIMZ_g5d2CUa0dinvbVGkYz8u9OyM4PiYquuxxg=9F090ED3'
...
const COMPLETION_DETAILS = {
  'COT-203': {
    ...
    files: ['src/stores/useAppStore.ts', 'src/lib/engine.ts', 'src/components/AiControlPanel.tsx']
  },
  'COT-204': {
    ...
    files: ['src/lib/engine.ts (lines 281-297)']
  },
  'COT-205': {
    ...
    files: ['src/stores/useAppStore.ts (line 400)']
  },
  'COT-504': {
    ...
    files: ['src/components/CoachChatPanel.tsx (line 50)']
  },
  'COT-505': {
    ...
    files: ['src/components/CoachPanel.tsx (lines 107-141)', 'src/App.tsx (lines 110-143)']
  },
  'COT-506': {
    ...
    files: ['src/components/Sidebar.tsx (line 161)']
  },
  'COT-507': {
    ...
    files: ['None (verified existing state)']
  }
}
...
async function getIssueTypes(projectKey) {
  try {
    ...
    const metadataResponse = await jiraRequest(
...
async function checkIfScrumIsCot() {
  try {
    ...
    const hasCotKeywords =
      issueSummaries.includes('chess') ||
    ...
    const hasNsKeywords =
      issueSummaries.includes('narrative') ||
...
async function renameProject(oldKey, newKey, newName, description = '') {
  try {
    ...
    if (error.message.includes('403') || error.message.includes('permission')) {
      throw new Error(
        `Permission denied: Administer Jira global permission required to change project key. ${error.message}`
      )
    }
    throw error
  }
...
async function createProject(
  ...
  try {
    ...
    if (error.message.includes('403') || error.message.includes('permission')) {
      throw new Error(
        `Permission denied: Create Projects global permission required. ${error.message}`
      )
    }
    if (error.message.includes('400') && error.message.includes('key')) {
      throw new Error(
        `Project key conflict: ${key} may already exist or be invalid. ${error.message}`
      )
    }
    throw error
  }
...
function createCompletionComment(completionData) {
  ...
  content.push({
    type: 'paragraph',
    content: [{ type: 'text', text: 'Completion Details:', marks: [{ type: 'strong' }] }]
  })
  ...
  if (completionData.files && completionData.files.length > 0) {
    ...
    content.push({
      type: 'paragraph',
      content: [{ type: 'text', text: 'Files Modified:', marks: [{ type: 'strong' }] }]
    })
...
async function updateCompletions(issueKeys, keyMapping = {}) {
  ...
  for (const issueKey of issueKeys) {
    ...
    if (!completionData) {
      console.warn(
        `⚠️  No completion data found for ${issueKey} (expected: ${expectedKey}), skipping...`
      )
...
async function createMissingCotTickets() {
  ...
  const ticketDefinitions = {
    ...
    'COT-504': {
      type: 'Bug',
      epic: 'COT-5',
      summary: 'Remove Redundant Connection Status Messages'
    },
...
;(async () => {
  try {
    ...
    console.log(
      `📋 Step 6: Updating ${ticketsToUpdate.length} COT tickets with completion details...\n`
    )
